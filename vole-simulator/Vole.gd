...
func create_colored_bg(label_text: String) -> ColorRect:
	...
	# Determine the background color based on the label text
	if label_text in identifiers:
		bg.color = Color(0.2, 0.2, 0.3)  # Special color for identifiers
	elif label_text == "00" or label_text == "0000":
		bg.color = Color(0.2, 0.2, 0.2)  # Default background color
	else:
		bg.color = Color(0.4, 0.2, 0.2)  # Highlighted color for non-zero values
